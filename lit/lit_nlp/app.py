...
TxProvStats = None
def compute_TxProvStats():
  print("COMPUTING TXPROVSTATS")
  global TxProvStats
  TxProvStats = lineage.InferTransformStats(
    log_file_pth='results_a2t_sst2/log.csv',
    features = ["morph", "pos_", "dep_", "static_sentiment", "contextual_sentiment"]
  )
  print("DONE COMPUTING TXPROVSTATS")
Thread(target=compute_TxProvStats).start()
...
class LitApp(object):
  ...
  def _get_txProv_traces(self, data, **unused_kw):
    ...
    queryApi = lineage.InferQuery()
    traces = queryApi.get_trace_of_outputs_indices(data['inputs'])
    return traces

  def _get_txProv_stats(self, data, **unused_kw):
    """Invoke lineage.InferQuery().get_trace**"""
    # stats = None
    # stats = TxProvStats.update_human_label_by_idx(1, 1)
    # check if the TxProvStats background task has finished
    # either wait or maybe can also raise exception e.g. not-finished, try-again-later
    try:
      stats = None
      for item in data['inputs']:
        stats = TxProvStats.update_human_label_by_idx(item['data']['idx'], item['data']['label'])
      return stats["static_sentiment"].to_json(orient="table")
    except:
      raise ValueError("TxProvStats not yet computed...")

  def _get_txProv_textfetch(self, data, **unused_kw):
    # check if the TxTextFetch background task has finished
    # either wait or maybe can also raise exception e.g. not-finished, try-again-later
    try:
      query = data['inputs'][0]['data']['text']
      linguistic_features = ["semantic", "syntactic", "morphological"] #, "phonological"]
      output = dict()
      for lf in linguistic_features:
          output[lf] = []
          ranking, scores = TxTextFetch.search_for_text(query, lf, top_n=3)
          corpus_ids, corpus_scores = TxTextFetch.search_for_ids(query, lf, top_n=3)
          for text_id, text, score in zip(corpus_ids, ranking, scores):
              output[lf].append(dict(score=round(score, 2), text_id=text_id, text=text))
      return output
    except:
      raise ValueError("TxTextFetch not yet computed...")

  def _get_txProv_amr(self, data, **unused_kw):
    try:
      return dict(
        penman_graphs=TxTextFetch.amr_featurizer.model.parse_sents(
          [TxTextFetch.amr_text[idx] for idx in data['inputs']]
        )
      )
    except:
      raise ValueError("TxTextFetch not yet computed...")
  ...
  def __init__(
    ...
    handlers = {
        ...
        '/get_txProv_stats': self._get_txProv_stats,
        '/get_txProv_textfetch': self._get_txProv_textfetch,
        '/get_txProv_amr': self._get_txProv_amr
    }
