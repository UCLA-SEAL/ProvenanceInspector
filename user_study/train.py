...
def train(cfg: DictConfig) -> None:
    ...
    os.makedirs(cfg.train.logging_dir, exist_ok=True)
    ...
    dataset_names = glob.glob(os.path.join(cfg.dataset_dir, cfg.train.dataset_matcher))
    dataset_names = [p.replace("\\", "/") for p in dataset_names] # fix for formatting issues in windows
    dataset_names = [p.split("/")[-1] for p in dataset_names]
    ...
    for run_arg in run_args[start_position:]:
        ...
        dataset_name = run_arg['dataset_name']
        ...
        if dataset_name != "orig":
            save_path = os.path.join(cfg.dataset_dir, dataset_name)
            if "csv" in dataset_name:
                train_dataset = load_from_disk("csv", data_files=save_path)
            else:
        ...
        output_dir = os.path.join(cfg.train.trained_models_dir, f"{base_model_rename}.{dataset_name}")
        ...
        results_save_name = f"{dataset_name}.csv"
        results_save_path = os.path.join(cfg.results_dir, results_save_name)
        log.info(f"Saving results to {results_save_path}")
